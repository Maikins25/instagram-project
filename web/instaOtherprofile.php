...
while($row = $result->fetch_assoc()){
    ...
    echo '<div class="yourProfile">';
        ...
        echo '<h1>'. $row['user_username'].'</h1>';

        echo'<div class="userProfile">';

        echo '<img class="userProfilePic" src="' . $image_path . $userId . '.png">';
...
left outer join likes on like_post_id = post_id and like_user_id = $userId
...
while ($row = $result->fetch_assoc())
{
    ...
    if($lastPostId != $row['post_id']){
        ...
        $liked = false;
        if($userId == $row['like_user_id'] and $row['post_id'] == $row['like_post_id']){
            $liked = true;
        }
